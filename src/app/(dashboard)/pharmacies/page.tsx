            ...
            <div className="flex flex-col items-start gap-5 md:flex-row md:items-center">
              ...
              <div className="flex-1">
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-blue-700">
                  ...
                  devez d’abord créer une pharmacie. Cette pharmacie aura son
                  propre nom, son responsable, son taux de change et ses
                  paramètres.
                </p>
              </div>
...
{canCreatePharmacy ? (
  <button
    type="button"
    onClick={() => setIsDialogOpen(true)}
    className="inline-flex items-center gap-2 rounded-2xl bg-blue-700 px-5 py-3 text-sm font-black text-white hover:bg-blue-800"
  >
    <Plus className="h-5 w-5" />
    Créer maintenant
  </button>
) : (
  <div className="rounded-2xl border border-amber-100 bg-white px-5 py-3 text-sm font-bold text-amber-700">
    Contactez l’administrateur Aksantic pour créer ou activer votre pharmacie.
  </div>
)}
            </div>
          </section>
...
function PharmacyCard({ pharmacy }: { pharmacy: PharmacyWithRole }) {
  return (
    <article className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
      ...
      <div className="mt-5">
        <h2 className="text-xl font-black text-slate-950">{pharmacy.name}</h2>
      ...
      <div className="mt-5 space-y-3 text-sm text-slate-600">
        ...
        <InfoLine
          ...
          value={`1 USD = ${Number(pharmacy.exchange_rate).toLocaleString(
            "fr-CD"
          )} CDF`}
        />
      </div>
